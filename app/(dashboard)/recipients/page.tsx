  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Destinataires</h1>
        <RecipientFormDialog onSaved={loadRecipients} countryId={selectedCountry?.id} />
      </div>

      <Card>
        <CardHeader>
          <CardTitle>
            {selectedCountry ? `Destinataires — ${selectedCountry.name}` : "Tous les destinataires"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <tbody>
                {recipients.map((r) => (
                  <tr key={r.id} className="border-b last:border-0">
                    ...
                    <td>
                      <Badge variant="outline">{GROUP_LABELS[r.group] ?? r.group}</Badge>
                    </td>
                    <td>
                    ...
                    <td className="space-x-1 whitespace-nowrap">
                      ...
                      />
                      <DeleteRecipientDialog recipient={r} onDeleted={loadRecipients} />
                    </td>
                  </tr>
                ))}
                {recipients.length === 0 && (
